...
inline constexpr int kFlagLayerCount = 7;
...
public:
    ...
    /// playback frame ``index``, optionally filtered by whether only infant-labeled
    /// hands should contribute (when per_track_coloring is false).
    /// Always false for an out-of-range frame or flag index.
    bool is_flagged(int index, int flag_index, bool per_track_coloring = true) const;
...
private:
    ...
    std::vector<std::array<bool, kFlagLayerCount>> frame_flags_infant_only_; // only infant hands contribute
    int frame_count_;
};
