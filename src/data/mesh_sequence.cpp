...
MeshSequence::MeshSequence(const std::string& folder, bool smooth) : folder_(folder) {
    ...
    // (possibly missing) ``tracks_`` file. Resolved before any smoothing so the raw
    // frames read below already carry their track ids (the smoother groups by them).
...
void MeshSequence::smooth_from_disk() {
    // Read the whole sequence once and keep it (raw_frames_) so the smoother can be
    // re-run with new params without touching disk. The forward-backward smoother
    // needs every frame, so this is an unavoidable up-front load (no on-demand
    // streaming for a smoothed sequence). The raw frames carry the track ids from
    // load_frame_from_disk, which the smoother groups hands by.
    raw_frames_.clear();
...
void MeshSequence::set_tracking_source(TrackingSource source) {
    ...
    // When smoothing, the new source changes which hands share a track, so re-read
    // the raw frames (now stamped with the new ids) and re-smooth against them.
    if (smoothed_) {
        smooth_from_disk();
    }
}
